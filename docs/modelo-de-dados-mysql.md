# Projeto de Banco de Dados MySQL: Documentos, Servidores e Unidades Gestoras

> Modelo relacional proposto para substituir a planilha de controle como fonte do painel de conformidade. Detalhes do contexto em [fontes-de-dados.md](./fontes-de-dados.md) e [arquitetura.md](./arquitetura.md).

## Visão Geral

Este documento propõe o modelo de banco de dados MySQL para gerenciar documentos, servidores responsáveis, unidades gestoras e o histórico de processamento desses documentos. O modelo foi desenvolvido a partir da análise da estrutura das extrações descritas em [fontes-de-dados.md](./fontes-de-dados.md), buscando otimizar a organização, o controle e a rastreabilidade das informações.
...
- Os `Documentos` estão vinculados a uma `Unidade Gestora` (UG).
...
```sql
-- 1 Selecionar todos os servidores
...
```sql
-- 4 Contar a quantidade de documentos por tipo
SELECT tipo_documento, COUNT(*) AS total_documentos FROM Documento GROUP BY tipo_documento;

-- 5 Buscar documentos criados em abril de 2025
...
## Trabalhos Futuros e Contato

A adoção do banco relacional é a evolução prevista para substituir a planilha de controle como fonte do painel, trazendo **integralidade, histórico de alterações e escalabilidade**. Os benefícios esperados estão detalhados em [../dashboard/README.md](../dashboard/README.md#melhorias-planejadas) e o plano de evolução em [arquitetura.md](./arquitetura.md).

**Contato**

| | |
| --- | --- |
| **Autor** | Victor de Melo |
| **E-mail** | `victotqp@hotmail.com` · `victorcupinxa@gmail.com` |
...
Este documento descreve um fluxo completo de trabalho em design de banco de dados, da concepção à consulta. As etapas que o compõem são:

1.  **Análise de Requisitos:** Identificação de entidades, atributos e relacionamentos a partir da estrutura dos dados reais.
...
- [governanca-de-dados.md](./governanca-de-dados.md) — classificação, qualidade e linhagem dos dados
- [fontes-de-dados.md](./fontes-de-dados.md) — catálogo das fontes de dados
- [arquitetura.md](./arquitetura.md) — arquitetura da solução e roadmap
- [../dashboard/README.md](../dashboard/README.md) — painéis e melhorias planejadas
